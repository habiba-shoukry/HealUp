...
function App() {
  ...
  const patchUserStats = useCallback(async (payload) => {
    ...
    try {
      const res = await fetch(`https://healup-gtgv.onrender.com/apistats/${userId}`, {
        method: 'PATCH',
  ...
  useEffect(() => {
    const userId = getCurrentUserId();
    if (!userId) return;
    fetch(`https://healup-gtgv.onrender.com/apistats/${userId}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    const loadAvatarProfile = async () => {
      try {
        const res = await fetch(`https://healup-gtgv.onrender.com/apiavatars/profile/${userId}`);
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const persistSelections = async () => {
      try {
        await fetch(`https://healup-gtgv.onrender.com/apiavatars/profile/${userId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
