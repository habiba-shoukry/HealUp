...
const ChallengesCard = ({ onViewAll, onChallengeComplete }) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://healup-gtgv.onrender.com/apichallenges?userId=${userId}&programType=${encodeURIComponent(selectedProgram)}`)
      .then((res) => res.json())
      .then((data) => {
...
const handleConsentDecline = () => {
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    const fetchMetrics = () => {
      fetch(`https://healup-gtgv.onrender.com/apimetrics/weekly/${userId}?device=${encodeURIComponent(activeDevice)}`)
