...
const Layout = ({ children, stats = { xp: 0, coins: 0 }, onDeviceSwitch }) => {
  ...
  //       const res = await fetch(`https://healup-gtgv.onrender.com/apinotifications?userId=${userId}`);
  ...
  const markAll = async () => {
    try {
      await Promise.all(
        notifications.map(n =>
          fetch(`https://healup-gtgv.onrender.com/apinotifications/${n._id}/read`, { method: "PATCH" })
        )
      );
  ...
  const markAsRead = async (id) => {
    try {
      await fetch(`https://healup-gtgv.onrender.com/apinotifications/${id}/read`, { method: "PATCH" });
  ...
  const deleteNotification = async (id) => {
    try {
      await fetch(`https://healup-gtgv.onrender.com/apinotifications/${id}`, { method: "DELETE" });
